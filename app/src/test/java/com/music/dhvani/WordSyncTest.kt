...
import com.music.dhvani.data.lyrics.EnhancedLrc
import com.music.dhvani.data.lyrics.LyricLine
import com.music.dhvani.data.lyrics.LyricWord
import com.music.dhvani.data.lyrics.LyricsPlus
import com.music.dhvani.data.lyrics.TtmlLyrics
...
class WordSyncTest {
    ...
    fun `a slowly sung line-synced song gets no break between its lines`() {
        val lines = lineSynced(
            Triple(19_740L, 10_020L, "??? ??? ?? ?????????? ?? ?? ??? ??? ?? ????"),
            Triple(29_760L, 9_910L, "?? ??? ??, ?????? ?? ?? ??????? ??? ?? ????"),
            Triple(39_670L, 10_000L, "???? ??????? ?? ?? ????-????"),
        )
