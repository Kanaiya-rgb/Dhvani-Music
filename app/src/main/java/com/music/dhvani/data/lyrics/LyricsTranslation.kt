 ...
 *
 * Requests are batched and the compact result is kept in a bounded cache under [Context.getCacheDir],
 * so Android may reclaim it and the feature can never grow without limit.
 */
object LyricsTranslation {
    ...
    suspend fun translate(
        ...
        val rawTranslated = complete.flatMap { it.translations }
        val translated = List(slots.size) { idx ->
            rawTranslated.getOrNull(idx)?.ifBlank { slots[idx].text } ?: slots[idx].text
        }
    ...
    private suspend fun requestBatch(batch: Batch, target: String): BatchAnswer? {
        ...
        val request = Request.Builder()
            .url(ENDPOINT)
            .header("User-Agent", "Dhvani-Music/2.0.7")
        ...
        val response = try {
            ...
            android.util.Log.w("LyricsTranslation", "Translation network error", e)
            return null
        }
        return runCatching {
            ...
            val count = batch.slots.size
            val reconciled = if (parts.size == count) {
                parts.mapIndexed { idx, part ->
                    part.ifBlank { batch.slots[idx].text }
                }
            } else if (parts.size > count) {
                parts.take(count).mapIndexed { idx, part ->
                    part.ifBlank { batch.slots[idx].text }
                }
            } else {
                List(count) { idx ->
                    parts.getOrNull(idx)?.ifBlank { batch.slots[idx].text } ?: batch.slots[idx].text
                }
            }
            android.util.Log.d("LyricsTranslation", "Batch translated: slots=$count parts=${parts.size} source=$source target=$target")
            BatchAnswer(reconciled, source, batch.payload.length)
        }.onFailure {
            android.util.Log.w("LyricsTranslation", "Failed to parse translation response", it)
        }.getOrNull()
    }
