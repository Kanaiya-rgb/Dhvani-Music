...
import com.music.flux.data.model.NOTIFICATION_ART_PX
...
import com.music.flux.ui.rememberIsForeground
...
fun rememberPlayerState(controller: MediaController?): PlayerState {
    ...
    LaunchedEffect(state.song?.videoId, state.isPlaying) {
        val currentSong = state.song
        if (currentSong != null && state.isPlaying) {
            PlaybackHistory.record(currentSong)
        }
    }
 ...
 */
private const val EXTRA_FROM_AUTOPLAY = "flux.fromAutoplay"
 ...
 */
private const val EXTRA_ARTIST_ID = "flux.artistId"
private const val EXTRA_ALBUM_ID = "flux.albumId"

/** @see Song.localUri */
private const val EXTRA_LOCAL_URI = "flux.localUri"

/** @see Song.localPath */
private const val EXTRA_LOCAL_PATH = "flux.localPath"
 ...
 * playback URI.
 */
private const val EXTRA_DURATION = "flux.durationText"
 ...
 * catalogue audio release by [com.music.flux.data.YtMusicRepository.resolveAudio]
 ...
 * terms [com.music.flux.data.sources.TrackMatcher] compares recordings on.
 *
 ...
 * — see [com.music.flux.data.TrackLog.about].
 *
